    ...
    public class MongoTarget : Target
    {
        ...
        private void AddDefaults(BsonDocument document, LogEventInfo logEvent)
        {
            ...
            if (logEvent.Level != null)
                document.Add("Level", BsonValue.Create(ToEnum(logEvent.Level)));
        ...
        public static Level ToEnum(LogLevel level)
        {
            if (level == LogLevel.Error) return Level.Error;
            if (level == LogLevel.Info) return Level.Information;
            if (level == LogLevel.Fatal) return Level.Critical;
            if (level == LogLevel.Warn) return Level.Warning;
